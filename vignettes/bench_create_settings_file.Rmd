...
* __VdistRec__: recovery time after disturbance (number of observations)
...
```{r}
  #-------------------------------------------------
  #  settings  
STnrep <- 10 # number of repetitions per parameter combination
STnobsYr <- 365
...
Vnyr <- seq(6,40,6)# number of years
VdistMag <- -seq(0.1,0.5,0.1)# magnitude disturbance
VdistT <- seq(3,18,3)# timing disturbance
VdistRec <- seq(3,15,2)*STnobsYr# recovery disturbance
Vqntl <- c(.05, .275, .5, .725, .95)# set of quantiles used to derive realistic values (for number of missing values, seasonal amplitude and standard deviation of the remainder) from sampled data
...
```{r}
save(sttngs, file = file.path(ofolder, paste0(basename, '_', pol, '_simTS_settings_var.rda')))
