...
```{r}
ifolder <- '/media/wanda/DATA/SimTS/pie1/' #'../Data/'#'C:\\Users\\keers001\\OneDrive\ -\ WageningenUR\\RETURN\\Data\\RETURN\\20191019_SimulationOptSAR\\Landsat\\SimTS\\pie1\\' # folder with data characteristics
ofolder <- '/home/wanda/Dropbox/RecInd/pie1/' #'../Data/'#'C:\\Users\\keers001\\OneDrive\ -\ WageningenUR\\RETURN\\Data\\RETURN\\20191019_SimulationOptSAR\\Landsat\\SimTS\\pie1' # folder where the simulated time series will be saved
ncores <-  8 # number of cores used for parallel computing
...
caseList <- c('nyr','seasAmp','remSd', 'distMag','distT', 'distRec', 'missVal')#c('distT','nDr', 'nyr','seasAmp','remSd', 'distMag','distT', 'distRec', 'missVal')#) c('distMag','missVal') # simulation cases to be evaluated
...
for(si in 1:length(caseList)){# # iterate over simulation cases
  ...
  m <- foreach(rset = 1:length(funSet[[1]]), .packages = 'BenchmarkRecovery') %dopar% {
    ...
    return(list(m_RRIi, m_R80pi, m_YrYri, m_SLi, s_RRIi, s_R80pi, s_YrYri, s_SLi))
  }
