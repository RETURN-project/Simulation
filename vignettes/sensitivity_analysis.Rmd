...
```{r extract-settings}
sttngs <- extract_settings(basename, ifolder, nyr, nobsYr, ofolder)
...
```{r simulate-and-extract, eval=FALSE}
...
caseList <- sttngs$general$eval # ("distT", "distRec", "missVal", "distMag", "seasAmp", "remSd")
results_list <- mclapply(caseList, FUN = evalParam, sttngs = sttngs, pars = pars, funSet = funSet, basename = basename, ofolder = ofolder, mc.cores = no_cores - 1)
