...
source_rmd('fun_eval_performance.Rmd')
```
...
#-------------------------------------------------
# inputs
ifolder <- 'C:\\Users\\keers001\\OneDrive\ -\ WageningenUR\\RETURN\\Data\\RETURN\\20191019_SimulationOptSAR\\Landsat\\Test_TSrequired\\TS_1\\RecInd\\' # folder with data characteristics
setfolder <- 'C:\\Users\\keers001\\OneDrive\ -\ WageningenUR\\RETURN\\Data\\RETURN\\20191019_SimulationOptSAR\\Landsat\\Test_TSrequired\\TS_1\\SimTS\\' # folder with data characteristics
figfolder <- 'C:\\Users\\keers001\\OneDrive\ -\ WageningenUR\\RETURN\\Data\\RETURN\\20191019_SimulationOptSAR\\Test_TSrequired\\Figures\\' # folder where the simulated time series will be saved
...
for (si in 1:length(simcases)){
  sname <- simcases[[si]]
  # plot R2
  print(ggplot(tot_rsq[[sname]], aes(variable,value,color=interaction(Smooth,Dense), group = Metric)) +
  ...
  # plot RMSE
  print(ggplot(tot_rmse[[sname]], aes(variable,value,color=interaction(Smooth,Dense), group = Metric)) +
          geom_line(aes(),size=2, alpha = 0.4)+#linetype=interaction(Dense,Smooth)+ 
          scale_color_manual('Preprocessing', values=c("#BC92C2", "#D62B2A", "#B8D464", "#5ACFE4", "#865C7C", "#7FAC5A", "#508EA8"))+ 
          ...
          theme(axis.text.x = element_text(color = "grey50", size = 20),
                ...
                strip.text.y = element_text(size = 20,color = "grey20")))
  # plot MAPE
  print(ggplot(tot_mape[[sname]], aes(variable,value,color=interaction(Smooth,Dense), group = Metric)) +
          geom_line(aes(),size=2, alpha = 0.4)+#linetype=interaction(Dense,Smooth)+ 
          scale_color_manual('Preprocessing', values=c("#BC92C2", "#D62B2A", "#B8D464", "#5ACFE4", "#865C7C", "#7FAC5A", "#508EA8"))+ 
          facet_grid(vars(Metric), vars(Period))+ 
          xlab(simFullName[[sname]]) +
