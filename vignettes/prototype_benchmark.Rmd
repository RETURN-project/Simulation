...
Vnyr <- c(6,40)# number of years
VdistMag <- -c(0.1,0.5)# magnitude disturbance
VdistT <- c(3,33)# timing disturbance
VdistRec <- c(0.5,5)*STnobsYr# recovery disturbance
Vqntl <- c(.05, .95)# set of quantiles used to derive realistic values (for number of missing values, seasonal amplitude and standard deviation of the remainder) from sampled data
...
VdistType <- 'exponential' #piecewise or exponential
...
```{r}
# recovery settings
caseList <- c( 'remSd') # the time series characteristic that is being evaluated
funSet <- list('freq' = c('annual', 'dense','dense','dense','dense','dense','dense'),
               'input' = c('raw','raw', 'smoothed', 'segmented', 'raw', 'smoothed', 'segmented'),
               ...
               'h' = c(0.15,0.15,0.15,0.15,0.15,0.15,0.15))# settings for the recovery indicators
...
for(vr in 1:length(caseList)){
  ...
  for (i in 1:length(setvr)){#length(setvr)     
    ...
    R80p_nTS[i,] <- apply(m_RRIi, 1, function(x){sum(is.na(x)==F)/length(x)})
    YrYr_nTS[i,] <- apply(m_RRIi, 1, function(x){sum(is.na(x)==F)/length(x)})
    SL_nTS[i,] <- apply(m_RRIi, 1, function(x){sum(is.na(x)==F)/length(x)})
  }   
  
  RRI_rsqDF <- toDF(RRI_rsq, setvr, 'RRI', funSet$freq, funSet$input, funSet$nDist)
  R80p_rsqDF <- toDF(R80p_rsq, setvr, 'R80p', funSet$freq, funSet$input, funSet$nDist)
  YrYr_rsqDF <- toDF(YrYr_rsq, setvr, 'YrYr', funSet$freq, funSet$input, funSet$nDist)
  SL_rsqDF <- toDF(SL_rsq, setvr, 'SL', funSet$freq, funSet$input, funSet$nDist)
  
  RRI_mapeDF <- toDF(RRI_mape, setvr, 'RRI', funSet$freq, funSet$input, funSet$nDist)
  R80p_mapeDF <- toDF(R80p_mape, setvr, 'R80p', funSet$freq, funSet$input, funSet$nDist)
  YrYr_mapeDF <- toDF(YrYr_mape, setvr, 'YrYr', funSet$freq, funSet$input, funSet$nDist)
  SL_mapeDF <- toDF(SL_mape, setvr, 'SL', funSet$freq, funSet$input, funSet$nDist)
  
  RRI_rmseDF <- toDF(RRI_rmse, setvr, 'RRI', funSet$freq, funSet$input, funSet$nDist)
  R80p_rmseDF <- toDF(R80p_rmse, setvr, 'R80p', funSet$freq, funSet$input, funSet$nDist)
  YrYr_rmseDF <- toDF(YrYr_rmse, setvr, 'YrYr', funSet$freq, funSet$input, funSet$nDist)
  SL_rmseDF <- toDF(SL_rmse, setvr, 'SL', funSet$freq, funSet$input, funSet$nDist)
  
  RRI_nTSDF <- toDF(RRI_nTS, setvr, 'RRI', funSet$freq, funSet$input, funSet$nDist)
  R80p_nTSDF <- toDF(R80p_nTS, setvr, 'R80p', funSet$freq, funSet$input, funSet$nDist)
  YrYr_nTSDF <- toDF(YrYr_nTS, setvr, 'YrYr', funSet$freq, funSet$input, funSet$nDist)
  SL_nTSDF <- toDF(SL_nTS, setvr, 'SL', funSet$freq, funSet$input, funSet$nDist)
...
```{r}
caseList <- c( 'remSd')# evaluated time series characteristics for which a plot needs to be made
...
for(vr in 1:length(caseList)){
  ...
  tot_rsq <- rbind(RRI_rsq, R80p_rsq, YrYr_rsq, SL_rsq)
  tot_rmse <- rbind(RRI_rmse, R80p_rmse, YrYr_rmse, SL_rmse)
  tot_mape <- rbind(RRI_mape, R80p_mape, YrYr_mape, SL_mape)
  tot_nTS <- rbind(RRI_nTS, R80p_nTS, YrYr_nTS, SL_nTS)
  
  melt(tot_rsq)
  
  sname <- caseList[[vr]]
  # plot R2
  print(ggplot(melt(tot_rsq), aes(variable,value,color=interaction(Smooth,Dense), group = interaction(Smooth,Dense))) +
  ...
  # plot RMSE
  print(ggplot(melt(tot_rmse), aes(variable,value,color=interaction(Smooth,Dense), group = interaction(Smooth,Dense))) +
          geom_line(aes(),size=2, alpha = 0.4)+#linetype=interaction(Dense,Smooth)+ 
          scale_color_manual('Preprocessing', values=c("#BC92C2", "#D62B2A", "#B8D464", "#5ACFE4", "#865C7C", "#7FAC5A", "#508EA8"))+ 
          ...
          theme(axis.text.x = element_text(color = "grey50", size = 20),
                ...
                strip.text.y = element_text(size = 20,color = "grey20")))
  # plot MAPE
  print(ggplot(melt(tot_mape), aes(variable,value,color=interaction(Smooth,Dense), group = interaction(Smooth,Dense))) +
          geom_line(aes(),size=2, alpha = 0.4)+#linetype=interaction(Dense,Smooth)+ 
          scale_color_manual('Preprocessing', values=c("#BC92C2", "#D62B2A", "#B8D464", "#5ACFE4", "#865C7C", "#7FAC5A", "#508EA8"))+ 
          facet_grid(vars(Metric), vars(Period))+ 
          xlab(simFullName[[sname]]) +
  ...
  print(ggplot(melt(tot_nTS), aes(variable,value,color=interaction(Smooth,Dense), group = interaction(Smooth,Dense))) +
          geom_line(aes(),size=2, alpha = 0.4)+#linetype=interaction(Dense,Smooth)+ 
          scale_color_manual('Preprocessing', values=c("#BC92C2", "#D62B2A", "#B8D464", "#5ACFE4", "#865C7C", "#7FAC5A", "#508EA8"))+ 
          facet_grid(vars(Metric), vars(Period))+ 
          xlab(simFullName[[sname]]) +
