...
```{r}
ifolder <- '../Data/char' #'C:\\Users\\keers001\\OneDrive\ -\ WageningenUR\\RETURN\\Data\\RETURN\\20191019_SimulationOptSAR\\Landsat\\Test_TSrequired\\TS_10\\SimTS\\'  # folder with data characteristics
ofolder <- '../Data/SimTS' # 'C:\\Users\\keers001\\OneDrive\ -\ WageningenUR\\RETURN\\Data\\RETURN\\20191019_SimulationOptSAR\\Landsat\\Test_TSrequired\\TS_10\\SimTS\\'  # folder where the simulated time series will be saved

basename <- 'toydata' # 'LSTS_RndmSample_NoFire_5_Tree_80_scl_30_npnt_20000_VI'# base name of the data files
```
...
sttngs <- loadRData(file = file.path(ifolder, paste0(basename, '_VI_simTS_settings_var.rda')))
...
for(vr in c(4,7)){#length(sttngs$variables) 5,7
  ...
  for (i in 1:length(sttngs[[evr]])){     
    ...
    for (pari in 1: dim(comb)[1]){
      ...
      sc <- simulCase(sttngs$nrep, comb$nyr[pari], sttngs$nobsYr, sttngs$nDr, sttngs$seasAv, comb$seasAmp[pari],
                      ...
                      comb$missVal[pari], sttngs$DistMissVal)
  ...
  save(simTS, file = file.path(ofolder, paste0(basename, '_VI_simTS_' , evr, '.rda')))# '_', i,
  save(simTSTr, file = file.path(ofolder, paste0(basename, '_VI_simTSTr_', evr, '.rda')))
  save(simTSSeas, file = file.path(ofolder, paste0(basename, '_VI_simTSSeas_', evr, '.rda')))
  save(simTSRem, file = file.path(ofolder, paste0(basename, '_VI_simTSRem_', evr, '.rda')))
  #save(simTSDist, file = file.path(ofolder, paste0(basename, '_VI_simTSDist_', evr, '_', i,'.rda')))
  save(simTSParam, file = file.path(ofolder, paste0(basename, '_VI_simTSParam_', evr, '.rda')))
